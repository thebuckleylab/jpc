...
def _plot_rescaling_vs_width_depth_single_seed(
    results_dir,
    plot_dir,
    seed,
    gamma_0,
    ...
    if widths_filter is not None and len(widths_filter) > 0:
        ...
        configs = {
            (w, h): path for (w, h), path in configs.items() if w in widths_filter_set
        }
    ...
    if n_hiddens_filter is not None and len(n_hiddens_filter) > 0:
        ...
        available_n_hiddens_before = {
            n_hidden if n_hidden is not None else 0
            for (w, n_hidden), path in configs.items()
        }
        print(
            f"  Available n_hidden values before filtering: {sorted(available_n_hiddens_before)}"
        )
        ...
        print(
            f"  Remaining n_hidden values after filtering: {sorted(remaining_n_hiddens)}"
        )
        ...
        if rescalings is not None and len(rescalings) > 0:
            ...
            if isinstance(first_rescaling, np.ndarray):
                if first_rescaling.ndim == 0:
                    ...
                    first_rescaling = float(
                        first_rescaling.item()
                        if first_rescaling.size == 1
                        else first_rescaling[0]
                    )
            else:
    ...
    if len(data_points) > 0:
        ...
        print(
            f"  Rescaling-1 range: {min(rescalings_minus_one):.6f} - {max(rescalings_minus_one):.6f}"
        )
    ...
    rescalings_arr_original = np.array(
        rescalings_minus_one
    )  # Keep original for linear scale
    ...
    if linear_z_scale:
        ...
        if min_rescaling <= 0:
            print(
                f"  Warning: Some rescaling-1 values are non-positive (min={min_rescaling:.6f}), adding small offset for log scale"
            )
    ...
    if param_type != "sp":
        ...
        if use_skips:
            theory_values_linear = 1.0 / (gamma_0**2 * widths_linear) + (
                depths_linear - 1
            ) / (gamma_0**2 * widths_linear)
        else:
    ...
    if param_type != "sp":
        ...
        if use_skips:
            Z_theory_linear = 1.0 / (gamma_0**2 * W_linear) + (L_linear - 1) / (
                gamma_0**2 * W_linear
            )
        else:
        ...
        try:
            ...
            try:
                ...
                surf = ax.plot_surface(
                    W_log2, L_log2_grid, Z_theory, alpha=0.3, color="gray", zorder=1
                )
    ...
    if output_dim > 1:
        ...
        ax.set_zlabel(
            r"$s(\boldsymbol{\theta}_t) - 1$", fontsize=FONT_SIZES["label"], labelpad=34
        )
    ...
    if param_type != "sp":
        handles.append(Patch(facecolor="gray", alpha=0.3, label=r"Theory ($L/N$)"))
...
def plot_rescaling_vs_width_depth(
    ...
    if gamma_0 is None:
        gamma_0 = extract_gamma_0_from_dir(
            results_dir,
...
def plot_error_heatmap(
    ...
    base_configs = discover_available_configs(
        results_dir, seed=seeds_to_use[0], param_type=param_type, use_skips=use_skips
    )
    ...
    if widths_filter is not None and len(widths_filter) > 0:
        ...
        base_configs = {
            (w, h): path
            for (w, h), path in base_configs.items()
            if w in widths_filter_set
        }
    ...
    for (width, n_hidden), base_dir_path in base_configs.items():
        ...
        for seed_val in seeds_to_use:
            ...
            seed_configs = discover_available_configs(
                results_dir, seed=seed_val, param_type=param_type, use_skips=use_skips
            )
                    ...
                    if rescalings is not None and len(rescalings) > 0:
                        ...
                        if isinstance(first_rescaling, np.ndarray):
                            if first_rescaling.ndim == 0:
                                ...
                                first_rescaling = float(
                                    np.linalg.norm(first_rescaling, ord=2)
                                )
                            else:
                                first_rescaling = float(
                                    first_rescaling.item()
    ...
    if use_skips:
        theory_values = 1.0 / (gamma_0**2 * widths_arr) + (depths_arr - 1) / (
            gamma_0**2 * widths_arr
        )
    else:
    ...
    print(
        f"  Absolute error range: {np.min(absolute_errors):.6e} - {np.max(absolute_errors):.6e}"
    )
    ...
    cbar.set_label(r"$|\text{error}|$", fontsize=55, rotation=270, labelpad=80)
...
def plot_energy_loss_surfaces(
    ...
    # Discover all available configurations
    base_configs = discover_available_configs(
        results_dir, seed=0, param_type=param_type, use_skips=use_skips
    )
    ...
    ax.set_ylabel("$L$", fontsize=FONT_SIZES["label"], labelpad=LABEL_PAD)
    ax.set_zlabel(
        r"$l(\boldsymbol{\theta})$", fontsize=FONT_SIZES["label"], labelpad=LABEL_PAD
    )
    ...
    handles = [
        Patch(
            facecolor="blue",
            alpha=0.5,
            label=r"$\mathcal{F}^*(\boldsymbol{\theta})$ (PC)",
        ),
        Patch(
            facecolor="red", alpha=0.5, label=r"$\mathcal{L}(\boldsymbol{\theta})$ (BP)"
        ),
    ]
    ax.legend(handles=handles, fontsize=FONT_SIZES["legend"], loc="upper right")
...
def plot_grad_cosine_similarity_vs_width_depth(
    results_dir,
    plot_dir,
    seeds=[0, 1, 2],
    param_type=None,
    ...
    for seed_val in seeds_to_plot:
            ...
            if os.path.exists(cosine_sim_path):
                ...
                if cosine_sims is not None and len(cosine_sims) > 0:
                    ...
                    if isinstance(first_cosine_sim, np.ndarray):
                        first_cosine_sim = float(
                            first_cosine_sim.item()
                            if first_cosine_sim.ndim == 0
                            else first_cosine_sim[0]
                        )
                    else:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(plot_dir, filename), bbox_inches="tight", pad_inches=0.9
        )
        plt.close()
...
def plot_grad_cosine_similarity_heatmap(
    results_dir,
    plot_dir,
    seed=0,
    param_type=None,
        ...
        for seed_val in seeds_to_use:
            ...
            if (width, n_hidden) in seed_configs:
                ...
                cosine_sim_path = os.path.join(
                    seed_dir_path, "grad_cosine_similarities.npy"
                )
                ...
                if os.path.exists(cosine_sim_path):
                    ...
                    if cosine_sims is not None and len(cosine_sims) > 0:
                        ...
                        if isinstance(cosine_sim_at_step, np.ndarray):
                            cosine_sim_at_step = float(
                                cosine_sim_at_step.item()
                                if cosine_sim_at_step.ndim == 0
                                else cosine_sim_at_step[0]
                            )
                        else:
    ...
    print(f"  Loaded {len(data_points)} data points")
    print(
        f"  Cosine similarity range: {np.min(cosine_similarities):.6f} - {np.max(cosine_similarities):.6f}"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(plot_dir, filename), bbox_inches="tight", pad_inches=0.9, dpi=300
    )
    plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot rescaling vs width and depth in 3D"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    for dataset in args.datasets:
        ...
        for use_skips in use_skips_values:
            ...
            plot_error_heatmap(
                ...
                seed=args.seeds[0]
                if args.seeds
                else 0,  # Use first seed for error heatmap (which averages over all seeds anyway)
                gamma_0=args.gamma_0,
                param_type=args.param_type,
                output_dim=args.output_dim,
                widths_filter=args.widths,
