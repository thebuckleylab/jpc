...
from plot_toy_results import extract_dataset_id, get_color_val, load_data_from_dir
...
def plot_losses(
    ...
    pc_n_hiddens_list = sorted(
        [
            n
            for n in n_hiddens
            if n in data_by_n_hidden and data_by_n_hidden[n].get("pc_train_losses")
        ]
    )
    bp_n_hiddens_list = sorted(
        [
            n
            for n in n_hiddens
            if n in data_by_n_hidden and data_by_n_hidden[n].get("bp_losses")
        ]
    )
    ...
    if pc_n_hiddens_list:
        legend_handles.append(
            Line2D([0], [0], color=pc_legend_color, linewidth=LINE_WIDTH, alpha=ALPHA)
        )
    ...
    if bp_n_hiddens_list:
        legend_handles.append(
            Line2D([0], [0], color=bp_color, linewidth=LINE_WIDTH, alpha=ALPHA)
        )
    ...
    all_L_values = sorted(
        [(n_hidden + 1) if n_hidden is not None else 1 for n_hidden in all_n_hiddens]
    )
...
def plot_losses_and_energies(
    ...
    pc_n_hiddens_list = sorted(
        [
            n
            for n in n_hiddens
            if n in data_by_n_hidden and data_by_n_hidden[n].get("pc_energies")
        ]
    )
    bp_n_hiddens_list = sorted(
        [
    ...
    plt.tight_layout()
    filename = (
        f"losses_and_energies_N{width}.pdf"
        if width is not None
        else "losses_and_energies.pdf"
    )
    plt.savefig(os.path.join(results_dir, filename), bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot scaling limits results for saddle regime"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        "--log_xaxis", default=False, help="Use log scale on x-axis (default: True)"
    )
    ...
    if args.use_skips is not None:
        ...
        for param_type in param_types:
            use_skips_for_type = discover_available_use_skips(
                args.results_dir, seed=args.seed, param_type=param_type
            )
        ...
        if not all_use_skips:
            ...
            use_skips_values = sorted(
                list(all_use_skips), reverse=True
            )  # True before False
    ...
    for param_type in param_types:
        ...
        for use_skips in use_skips_values:
            ...
            skip_status_dir = os.path.join(
                param_type_plot_dir, f"{use_skips}_use_skips"
            )
            ...
            for width in args.widths:
                ...
                for n_hidden in n_hiddens:
                    ...
                    if data:
                        ...
                        if (
                            width in data.get("pc_train_losses", {})
                            or width in data.get("pc_energies", {})
                            or width in data.get("bp_losses", {})
                        ):
                            has_data = True
                ...
                if gamma_0 is None:
                    ...
                    print(
                        f"  Warning: gamma_0 not found in data or args, using default {gamma_0}"
                    )
                ...
                print(
                    f"  n_hiddens with data: {sorted([n for n in data_by_n_hidden.keys() if n is not None])}"
                )
                # Check for DMFT loss
                has_dmft = any(
                    data.get("dmft_loss") is not None
                    for data in data_by_n_hidden.values()
                )
                ...
                if args.plot == "all" or args.plot == "energies":
                    ...
                    print(
                        f"  Saved to {os.path.join(gamma_0_plot_dir, 'losses_and_energies.pdf')}"
                    )
