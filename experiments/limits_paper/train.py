...
from theory_utils import get_Delta, solve_kernels
from utils import (
                    ...
                    for param_type in args.param_types:
                        ...
                        for activity_lr in args.activity_lrs:
                            ...
                            for width in args.widths:
                                print(
                                    f"\t\t\t\t\tNumerical simulation for width N = {width}"
                                )

                                n_infer_iters = (
                                    args.n_infer_iters
                                    if (args.infer_mode == "closed_form")
                                    else n_hidden * 100
                                )
                                ...
                                for i in range(len(pc_model)):
                                    ...
                                    if not jnp.allclose(
                                        pc_weight, bp_weight, atol=1e-10
                                    ):
                                        all_match = False
                                        break
                                if all_match:
                                    print(
                                        f"\t\t\t\t\t✓ PC and BP models have identical random initialization\n"
                                    )
                                else:
                                    print(
