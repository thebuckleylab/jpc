    ...
    if gammas_list:
        ...
        for idx, gamma_0 in enumerate(gammas_list):
            ...
            plt.plot(
                iterations,
                values,
                label=label_prefix.format(gamma_0),
                alpha=ALPHA,
...
def discover_available_gamma_0s(
    results_dir,
    width,
    seed=0,
    n_hidden=None,
...
def load_data_from_dir(
    results_dir, gamma_0s, width, seed=0, n_hidden=None, param_type=None, use_skips=None
):
    ...
    use_adam = any(
        ...
        and (param_type_str is None or param_type_str in root)
    ...
    if not use_adam:
        ...
        for gamma_0 in gamma_0s:
            ...
            loaded = False
            for path in [
                f"dmft_loss_{gamma_0}_gamma_0_seed_{seed}.npy",
        ...
        if not data["dmft_losses"] and n_hidden is not None:
            ...
            for path in [
                f"dmft_loss_{dataset_id}_n_hidden_{n_hidden}_seed_{seed}.npy",
    ...
    for gamma_0 in gamma_0s:
        ...
        for dir_path, files in dir_map.items():
            if (
                width_str in dir_path
                and gamma_0_str in dir_path
                and dir_path.split(os.sep)[-1] == seed_str
            ):
...
def plot_rescalings(
    data, results_dir, colormap_name="viridis", n_hidden=None, output_dim=1
):
...
def plot_losses_and_energies(
            ...
            if "dmft_losses" in data and data["dmft_losses"]:
                for idx, gamma_0 in enumerate(all_gammas_for_dmft):
                    if gamma_0 in data["dmft_losses"]:
                        ...
                        grey_val = (
                            0.8 - (idx / (n_gammas_for_dmft - 1)) * 0.6
                            if n_gammas_for_dmft > 1
                            else 0.5
                        )
    ...
    plt.ylabel(
        r"$\mathcal{F}^*(\boldsymbol{\theta}_t)$",
        ...
        for idx, gamma_0 in enumerate(all_gammas):
            ...
            legend_elements.append(
                Line2D(
                    ...
                    linestyle="-",
                    linewidth=LINE_WIDTH,
                    label=f"$\gamma_0 = {gamma_0}$",
                )
            )
    if plot_theory and (
        ("dmft_losses" in data and data["dmft_losses"]) or data["dmft_loss"] is not None
    ):
        legend_elements.append(
            Line2D(
                [0],
...
def plot_loss_energy_ratio(
    ...
    if gammas_list:
        ...
        for idx, gamma_0 in enumerate(gammas_list):
            energies = np.array(data["pc_energies"][gamma_0]).flatten()
            losses = np.array(
                data["pc_train_losses" if loss_type == "pc" else "bp_losses"][gamma_0]
            ).flatten()
...
def plot_losses(
    ...
    # Get all available gamma_0s for both PC and BP
    pc_gammas_list = sorted(
        [g for g in data["gamma_0s"] if g in data["pc_train_losses"]]
    )
    bp_gammas_list = sorted([g for g in data["gamma_0s"] if g in data["bp_losses"]])
    all_gammas = sorted(set(pc_gammas_list + bp_gammas_list))
    n_gammas = len(all_gammas) if all_gammas else 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot learning regimes results (varying gamma_0)"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        "--width", type=int, default=2048, help="Width N to use for all plots"
    )
    parser.add_argument(
        "--gamma_0s",
        type=float,
    ...
    for param_type in param_types:
        ...
        for use_skips in use_skips_values:
            ...
            if args.gamma_0s is None:
                ...
                print(
                    f"  Filtering by param_type: {param_type}, use_skips: {use_skips}"
                )
                ...
                if not gamma_0s:
                    print(
                        f"Warning: No gamma_0 values found for width {args.width} with param_type={param_type}, use_skips={use_skips}"
                    )
            ...
            dmft_status = (
                "Yes"
                if (
                    ("dmft_losses" in data and data["dmft_losses"])
                    or data.get("dmft_loss") is not None
                )
                else "No"
            )
            ...
            if "dmft_losses" in data and data["dmft_losses"]:
                print(
                    f"  DMFT loss loaded for gamma_0s: {sorted(data['dmft_losses'].keys())}"
                )
            elif data.get("dmft_loss") is not None:
                print(
                    f"  DMFT loss loaded (single value, will be used for all gamma_0s)"
                )
            ...
            plot_configs = [
                (
                    "rescalings",
                    plot_rescalings,
                    [data, plot_dir, args.colormap, args.n_hidden, args.output_dim],
                    "pc_rescaling.pdf",
                ),
                (
                    "energies",
            ...
            if args.plot == "all" or args.plot == "ratio":
                for loss_type, filename in [
                    ...
                    plot_loss_energy_ratio(
                        data, plot_dir, args.colormap, loss_type, args.n_hidden
                    )
                    print(f"  Saved to {os.path.join(plot_dir, filename)}")
            ...
            if args.plot == "all" or args.plot == "cosine":
                ...
                if similarities:
                    ...
                    plot_cosine_similarity(
                        data, similarities, plot_dir, args.colormap, args.n_hidden
                    )
                    print(
