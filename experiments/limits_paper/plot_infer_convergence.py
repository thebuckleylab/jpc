...
def load_energies_by_infer_mode(
    ...
    activity_lr_strs = (
        [f"{lr}_activity_lr" for lr in activity_lrs]
        if activity_lrs is not None
        else None
    )
    seed_str = str(seed)
...
def plot_energy_comparison(
    data, results_dir, param_type, n_hidden, colormap_name="viridis"
):
    ...
    if not common_widths:
        print(
            f"  Warning: No common widths found for param_type={param_type}, n_hidden={n_hidden}"
        )
        plt.close()
        return
    ...
    legend_elements = [
        Line2D(
            ...
            linewidth=4,
            alpha=0.8,
            label=r"Theory, $\mathcal{F}(\mathbf{z}^*)$",
        ),
        Line2D(
            [0],
            [0],
            ...
            color=sim_color,
            linewidth=LINE_WIDTH,
            label=r"Simulation, $\mathcal{F}(\mathbf{z}_{T_{\text{max}}})$",
        ),
    ]
...
def plot_energy_comparison_combined(
    all_data_dict, results_dir, param_type, colormap_name="viridis"
):
    ...
    n_hiddens = sorted(
        [n for n in all_data_dict.keys() if all_data_dict[n] is not None]
    )
    ...
    legend_elements = [
        ...
        Line2D(
            ...
            [0],
            linestyle="-",
            color=base_sim_color,
            linewidth=LINE_WIDTH,
            label=r"Sim., $\mathcal{F}(\mathbf{z}_{T_{\text{max}}})$",
        ),
    ]
...
def plot_energy_delta(all_data_dict, results_dir, param_type, colormap_name="viridis"):
    ...
    for idx, n_hidden in enumerate(n_hiddens):
        ...
        color = colormap(
            0.15 + (idx / max(n_depths - 1, 1)) * 0.85 if n_depths > 1 else 0.15
        )
    ...
    legend_handles.append(
        Line2D([0], [0], color="black", linewidth=4, linestyle="--", alpha=0.8)
    )
    ...
    if legend_handles:
        plt.legend(
            handles=legend_handles, labels=legend_labels, fontsize=FONT_SIZES["legend"]
        )
    save_plot(results_dir, f"energy_delta_{param_type}.pdf", None)


def plot_energy_delta_combined(
    all_data_dict_sp, all_data_dict_mupc, results_dir, colormap_name="viridis"
):
    ...
    n_hiddens_sp = sorted(
        [n for n in all_data_dict_sp.keys() if all_data_dict_sp[n] is not None]
    )
    n_hiddens_mupc = sorted(
        [n for n in all_data_dict_mupc.keys() if all_data_dict_mupc[n] is not None]
    )
                        ...
                        if use_skips_str is None or use_skips_str in dir_path:
                            ...
                            if act_fn_str is None or act_fn_str in dir_path:
                                ...
                                if infer_mode_str and infer_mode_str in dir_path:
                                    if activity_lr_strs is None or any(
                                        lr_str in dir_path
                                        for lr_str in activity_lr_strs
                                    ):
                                        if "energies.npy" in files:
                                            pc_dir = dir_path
...
def plot_losses_and_energies(
    all_data_dict, results_dir, param_type, infer_mode, colormap_name="viridis"
):
    ...
    if not n_hiddens:
        print(
            f"  Warning: No data found for param_type={param_type}, infer_mode={infer_mode}"
        )
        plt.close()
        return
    ...
    for idx, n_hidden in enumerate(n_hiddens):
        ...
        widths_with_energies = sorted(
            [w for w in data["widths"] if w in data["energies"]]
        )
    ...
    for idx, n_hidden in enumerate(n_hiddens):
        data = all_data_dict[n_hidden]
        # Use first available width
        widths_with_bp_losses = sorted(
            [w for w in data["widths"] if w in data["bp_losses"]]
        )
        if widths_with_bp_losses:
            ...
            plt.plot(
                iterations, losses, "--", alpha=ALPHA, linewidth=LINE_WIDTH, color=color
            )
    ...
    legend_elements = [
        ...
        Line2D(
            [0],
            [0],
            color=bp_middle_color,
            linestyle="--",
            linewidth=LINE_WIDTH,
            label=r"$\mathcal{L}(\boldsymbol{\theta})$ (BP)",
        ),
    ]
...
def plot_cosine_similarity(
    ...
    for idx, n_hidden in enumerate(n_hiddens):
        data = all_data_dict[n_hidden]
        # Use first available width
        widths_with_cosine = sorted(
            [w for w in data["widths"] if w in data["cosine_similarities"]]
        )
    ...
    if param_type == "mupc" and infer_mode == "optim":
        plt.axhline(
            y=1, color="black", linestyle="--", linewidth=4, alpha=0.8, zorder=10
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--widths", type=int, nargs="+", default=[2048], help="List of widths N to plot"
    )
    parser.add_argument(
        "--seed", type=int, default=0, help="Seed value for loading data"
    )
    parser.add_argument(
        "--colormap", type=str, default="Greens", help="Colormap to use for plotting"
    )
    parser.add_argument(
        "--n_hiddens",
        type=int,
    ...
    for use_skips in use_skips_values:
        ...
        for param_type in args.param_types:
            ...
            for n_hidden in args.n_hiddens:
                ...
                if not data["theory_energies"] or not data["simulation_energies"]:
                    print(
                        f"    Warning: Missing data for param_type={param_type}, n_hidden={n_hidden}, use_skips={use_skips}"
                    )
                    print(
                        f"      Theory energies: {sorted(data['theory_energies'].keys())}"
                    )
                    print(
                        f"      Simulation energies: {sorted(data['simulation_energies'].keys())}"
                    )
                ...
                plot_energy_comparison(
                    data, plot_subdir, param_type, n_hidden, args.colormap
                )
                print(
                    f"      Saved to {os.path.join(plot_subdir, f'energy_comparison_{param_type}_H{n_hidden}.pdf')}"
                )

            # Generate combined energy comparison plot (all depths)
            print(f"  Generating combined energy comparison plot (all depths)...")
            plot_energy_comparison_combined(
                all_data_dict, skip_status_dir, param_type, args.colormap
            )
            print(
                f"    Saved to {os.path.join(skip_status_dir, f'energy_comparison_{param_type}_combined.pdf')}"
            )
            ...
            print(
                f"    Saved to {os.path.join(skip_status_dir, f'energy_delta_{param_type}.pdf')}"
            )
        ...
        if all_data_dict_sp and all_data_dict_mupc:
            ...
            print(f"{'=' * 60}\n")
            plot_energy_delta_combined(
                all_data_dict_sp, all_data_dict_mupc, skip_status_dir, args.colormap
            )
            print(
                f"  Saved to {os.path.join(skip_status_dir, 'energy_delta_combined.pdf')}"
            )
        ...
        for param_type in args.param_types:
            for infer_mode in ["closed_form", "optim"]:
                ...
                if all_data_dict:
                    ...
                    print(
                        f"    Saved to {os.path.join(skip_status_dir, f'losses_and_energies_{param_type}_{infer_mode}.pdf')}"
                    )
        ...
        for param_type in args.param_types:
            for infer_mode in ["closed_form", "optim"]:
                ...
                if all_data_dict:
                    ...
                    print(
                        f"    Saved to {os.path.join(skip_status_dir, f'cosine_similarity_{param_type}_{infer_mode}.pdf')}"
                    )
