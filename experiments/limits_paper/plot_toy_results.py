    ...
    if widths_list:
        ...
        for idx, width in enumerate(widths_list):
            ...
            plt.plot(
                iterations,
                values,
                label=label_prefix.format(width),
                alpha=ALPHA,
...
def _load_npy_safe(path, flatten=True):
    ...
    if flatten and isinstance(arr, np.ndarray):
        if arr.dtype == object:
            return np.array(
                [
                    x.item() if hasattr(x, "item") else (float(x) if flatten else x)
                    for x in arr
                ]
            )
        return arr.flatten()
...
def load_data_from_dir(
    results_dir, widths, seed=0, n_hidden=None, param_type=None, use_skips=None
):
    ...
    use_adam = any(
        "adam_param_optim" in root or "adam_optim_id" in root
    ...
    if not use_adam:
        ...
        for search_dir in search_locations:
            ...
            for file in os.listdir(search_dir):
                if file.startswith("dmft_loss_") and file.endswith(
                    f"_seed_{seed_str}.npy"
                ):
                    # Check if it matches the gamma_0 pattern: dmft_loss_{gamma_0}_gamma_0_seed_{seed}.npy
                    file_base = file.replace("dmft_loss_", "").replace(
                        f"_seed_{seed_str}.npy", ""
                    )
    ...
    for width in widths:
        ...
        if bp_dir:
            for key, filename in [
                ("bp_losses", "losses.npy"),
                ("bp_grads", "bp_grads.npy"),
            ]:
                arr = _load_npy_safe(
                    os.path.join(bp_dir, filename), flatten=(key == "bp_losses")
                )
                if arr is not None:
                    data[key][width] = arr
...
def plot_rescaling_vs_width(
    ...
    gamma_0=None,
    seed=0,
    use_all_widths=False,
    data_results_dir=None,
    output_dim=1,
    param_type=None,
    use_skips=None,
):
    ...
    if use_all_widths:
        if data_results_dir is None:
            print(
                "  Warning: use_all_widths=True but data_results_dir not provided, falling back to data widths"
            )
            ...
            all_widths = discover_available_widths(
                data_results_dir,
                seed=seed,
                n_hidden=n_hidden,
            ...
            if all_widths:
                ...
                param_type_str = f"{param_type}_param_type" if param_type else None
                use_skips_str = (
                    f"{use_skips}_use_skips" if use_skips is not None else None
                )
    ...
    if output_dim > 1:
        plt.ylabel(
            r"$\|\mathbf{S}(\boldsymbol{\theta}_t)\|_2 - 1$",
            fontsize=FONT_SIZES["label"],
            labelpad=LABEL_PAD,
        )
    else:
        plt.ylabel(
            r"$s(\boldsymbol{\theta}_t) - 1$",
            fontsize=FONT_SIZES["label"],
            labelpad=LABEL_PAD,
        )
...
def plot_losses_and_energies(
    ...
    if data["dmft_loss"] is not None and param_type != "sp" and use_skips != True:
        ...
        plt.plot(
            ...
            color="black",
            linewidth=4,
            label=r"Theory ($N \rightarrow \infty$)",
            alpha=0.8,
        )
    ...
    if widths_list:
        legend_handles.append(
            plt.Line2D(
                [0], [0], color=pc_legend_color, linewidth=LINE_WIDTH, alpha=ALPHA
            )
        )
    ...
    if max_width in data["bp_losses"]:
        legend_handles.append(
            plt.Line2D([0], [0], color=bp_color, linewidth=LINE_WIDTH, alpha=ALPHA)
        )
    ...
    all_widths = sorted(
        set(
            [w for w in widths_list]
            + ([max_width] if max_width in data["bp_losses"] else [])
        )
    )
    n_all_widths = len(all_widths)
    for idx, width in enumerate(all_widths):
        gray_val = (
            0.3 + (idx / max(n_all_widths - 1, 1)) * 0.5 if n_all_widths > 1 else 0.5
        )
        gray_color = gray_cmap(gray_val)
        legend_handles.append(
            plt.Line2D([0], [0], color=gray_color, linewidth=LINE_WIDTH)
        )
    ...
    if data["dmft_loss"] is not None and param_type != "sp" and use_skips != True:
        legend_handles.append(
            plt.Line2D([0], [0], color="black", linewidth=4, linestyle="--", alpha=0.8)
        )
    ...
    plt.ylabel(
        r"$l(\boldsymbol{\theta}_t)$", fontsize=FONT_SIZES["label"], labelpad=LABEL_PAD
    )
    if log_x_scale:
        plt.xscale("log", base=10)
    if legend_handles:
...
def plot_loss_energy_ratio(
    data, results_dir, colormap_name="viridis", loss_type="pc", n_hidden=None
):
    ...
    if widths_list:
        ...
        for idx, width in enumerate(widths_list):
            energies = np.array(data["pc_energies"][width]).flatten()
            losses = np.array(
                data["pc_train_losses" if loss_type == "pc" else "bp_losses"][width]
            ).flatten()
...
def plot_bp_pc_losses_comparison(
    data,
    plot_dir,
    colormap_name="viridis",
    n_hidden=None,
    ...
    if data_results_dir is not None:
        ...
        all_available_widths = discover_available_widths(
            data_results_dir,
            seed=seed,
            n_hidden=n_hidden,
        ...
        if all_available_widths:
            print(
                f"  Found {len(all_available_widths)} widths with loss data: {all_available_widths}"
            )
    ...
    if narrow_width is not None:
        if narrow_width not in all_widths:
            print(
                f"  Warning: Specified narrow width {narrow_width} not available. Available widths: {all_widths}"
            )
    ...
    losses_data = {
        "pc_train_losses": data["pc_train_losses"].copy(),
        "bp_losses": data["bp_losses"].copy(),
    }
    if data_results_dir is not None:
        for width in [min_width, max_width]:
            if (
                width not in losses_data["pc_train_losses"]
                or width not in losses_data["bp_losses"]
            ):
                seed_str = str(seed)
                n_hidden_str = f"{n_hidden}_n_hidden" if n_hidden else None
                param_type_str = f"{param_type}_param_type" if param_type else None
                        ...
                        if n_hidden_str is None or n_hidden_str in root:
                            ...
                            if param_type_str is None or param_type_str in root:
                                ...
                                if use_skips_str is None or use_skips_str in root:
                                    ...
                                    if (
                                        "train_losses.npy" in files
                                        and width not in losses_data["pc_train_losses"]
                                    ):
                                        arr = _load_npy_safe(
                                            os.path.join(root, "train_losses.npy")
                                        )
                                        if arr is not None:
                                    ...
                                    if (
                                        "losses.npy" in files
                                        and width not in losses_data["bp_losses"]
                                    ):
                                        arr = _load_npy_safe(
                                            os.path.join(root, "losses.npy")
                                        )
                                        if arr is not None:
    ...
    for width, prefix in [(min_width, "min"), (max_width, "max")]:
        ...
        if width in losses_data["pc_train_losses"]:
            ...
            plt.plot(
                ...
                pc_loss,
                "-",
                color=colors[f"pc_{color_prefix}"],
                linewidth=4.5,
                label=f"PC, $N = {width}$",
                alpha=0.85,
            )
        ...
        if width in losses_data["bp_losses"]:
            ...
            plt.plot(
                ...
                bp_loss,
                "--",
                color=colors[f"bp_{color_prefix}"],
                linewidth=4.5,
                label=f"BP, $N = {width}$",
                alpha=0.85,
            )
...
def calculate_gradient_metrics(data):
    ...
    metrics = {"rescaled_loss_grad_norms": {}, "extra_energy_grad_norms": {}}
    ...
    for width in data["widths"]:
        ...
        rescalings = np.array(
            data["pc_rescalings"][width]
        )  # Shape: (n_iterations,) - scalar per iteration
        ...
        rescaled_loss_grads = (
            bp_grads / rescalings[:, np.newaxis]
        )  # Shape: (n_iterations, n_params)
...
def plot_gradient_norms(
    data, metrics, results_dir, colormap_name="viridis", n_hidden=None
):
    """Plot norms of rescaled loss gradient and extra energy gradient term."""
    plots = [
        (
            "rescaled_loss_grad_norms",
            r"$\left\|\frac{1}{s(\boldsymbol{\theta})}\frac{\partial \mathcal{L}}{\partial \boldsymbol{\theta}}\right\|$",
            "rescaled_loss_grad_norms.pdf",
        ),
        (
            "extra_energy_grad_norms",
            r"$\left\|\frac{1}{s(\boldsymbol{\theta})^2} \mathcal{L} \frac{\partial s}{\partial \boldsymbol{\theta}}\right\|$",
            "extra_energy_grad_norms.pdf",
        ),
    ]
...
def plot_cosine_similarity(
    data, similarities, results_dir, colormap_name="viridis", n_hidden=None
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="Seed value for loading data"
    )
    parser.add_argument(
        "--plot",
        type=str,
    ...
    def parse_use_skips(value):
        ...
        if lower_val in ("true", "1", "yes"):
            ...
            raise ValueError(
                f"Invalid use_skips value: {value}. Must be True/False, true/false, 1/0, or yes/no."
            )
    ...
    for dataset in args.datasets:
        ...
        for param_type in param_types:
            ...
            for n_hidden in n_hiddens:
                ...
                if n_hidden is not None:
                    print(
                        f"Processing plots for H = {n_hidden}, param_type = {param_type}"
                    )
                else:
                    print(
                        f"Processing plots (no n_hidden filter), param_type = {param_type}"
                    )
                print(f"{'=' * 60}\n")
                ...
                if n_hidden is not None:
                    n_hidden_dir = os.path.join(
                        dataset_plot_dir, f"{n_hidden}_n_hidden"
                    )
                else:
                ...
                for use_skips in use_skips_values:
                    ...
                    print(f"\n{'=' * 50}")
                    print(
                        f"Processing plots for use_skips = {use_skips}, param_type = {param_type}"
                    )
                    if n_hidden is not None:
                        print(f"H = {n_hidden}")
                    print(f"{'=' * 50}\n")
                    ...
                    print(
                        f"Loading data from {dataset_results_dir} (param_type={param_type}, use_skips={use_skips})..."
                    )
                    ...
                    print(f"\nLoaded data summary:")
                    print(
                        f"  DMFT loss: {'Yes' if data['dmft_loss'] is not None else 'No'}"
                    )
                    print(f"  PC rescalings: {sorted(data['pc_rescalings'].keys())}")
                    print(f"  PC energies: {sorted(data['pc_energies'].keys())}")
                    print(
                        f"  PC train losses: {sorted(data['pc_train_losses'].keys())}"
                    )
                    ...
                    print(
                        f"  Cosine similarities: {sorted(data['grad_cosine_similarities'].keys())}"
                    )
                    print()
                    ...
                    plot_configs = [
                        (
                            "rescalings",
                            plot_rescalings,
                            [data, plot_dir, args.colormap, None, args.output_dim],
                            "pc_rescaling.pdf",
                        ),
                        (
                            "rescaling_vs_width",
                            plot_rescaling_vs_width,
                            [
                                data,
                                plot_dir,
                                args.colormap,
                        ...
                        (
                            "comparison",
                            plot_bp_pc_losses_comparison,
                            [
                                data,
                                plot_dir,
                                args.colormap,
                                n_hidden,
                                args.narrow_width_comparison,
                                args.seed,
                                dataset_results_dir,
                                param_type,
                                use_skips,
                            ],
                            "bp_pc_losses_comparison.pdf",
                        ),
                    ]
                    ...
                    for plot_type, plot_func, plot_args, filename in plot_configs:
                        if args.plot == "all" or args.plot == plot_type:
                            ...
                            plot_func(*plot_args)
                            if (
                                plot_type == "rescaling_vs_width"
                                and data.get("gamma_0") is not None
                            ):
                                print(
                                    f"  Using gamma_0 = {data['gamma_0']} from data directory"
                                )
                            print(f"  Saved to {os.path.join(plot_dir, filename)}")

                    if args.plot == "all" or args.plot == "ratio":
                        for loss_type, filename in [
                            ("pc", "pc_loss_energy_ratio.pdf"),
                            ("bp", "bp_loss_energy_ratio.pdf"),
                        ]:
                            print(f"Generating {loss_type} loss/energy ratio plot...")
                            plot_loss_energy_ratio(
                                data, plot_dir, args.colormap, loss_type, None
                            )
                            print(f"  Saved to {os.path.join(plot_dir, filename)}")
                    ...
                    if args.plot == "all" or args.plot == "cosine":
                        ...
                        if similarities:
                            ...
                            plot_cosine_similarity(
                                data, similarities, plot_dir, args.colormap, None
                            )
                            print(
                                f"  Saved to {os.path.join(plot_dir, 'grads_cosine_similarities.pdf')}"
                            )
                        else:
