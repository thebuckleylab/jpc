    ...
    def __init__(
        ...
        for i in range(L):
            ...
            layer = nn.Sequential(
                [
                    nn.Lambda(act_fn_l),
                    nn.Linear(_in, _out, use_bias=use_bias, key=keys[i]),
                ]
            )
        ...
        if self.param_type == "depth_mup":
            for i, f in enumerate(self.layers):
                if (i + 1) == 1:
                    ...
                    rescaling = (
                        jnp.sqrt(self.N * self.L)
                        if self.use_skips
                        else jnp.sqrt(self.N)
                    )
...
def test_fwd_pass(
    ...
    if param_type == "depth_mup":
        model = init_weights(model=model, init_fn_id="standard_gauss", key=keys[1])
    ...
    for t, (img_batch, label_batch) in enumerate(train_loader):
        ...
        param_l2_norms[:, t] = compute_param_l2_norms(
            model=model.layers, act_fn=act_fn, layer_idxs=layer_idxs
        )
        param_spectral_norms[:, t] = compute_param_spectral_norms(
            model=model.layers, act_fn=act_fn, layer_idxs=layer_idxs
        )
        model, opt_state, _ = make_step(
            model=model, optim=optim, opt_state=opt_state, x=img_batch, y=label_batch
        )
        if t >= (n_checks - 1):
            break

    return (avg_activity_l1, avg_activity_l2, param_l2_norms, param_spectral_norms)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--param_types", type=str, nargs="+", default=["sp", "depth_mup", "orthogonal"]
    )
        ...
        for optim_id in args.optim_ids:
            ...
            for param_type in args.param_types:
                ...
                for use_skips in skip_uses:
                    ...
                    avg_activity_l1_per_N_L = np.zeros(
                        (N_RECORDED_LAYERS, N_CHECKS, len(WIDTHS), len(DEPTHS))
                    )
                    ...
                    param_spectral_norms_per_N_L = np.zeros_like(
                        avg_activity_l1_per_N_L
                    )
                    ...
                    for w, width in enumerate(WIDTHS):
                        ...
                        for d, depth in enumerate(DEPTHS):
                            ...
                            param_spectral_norms_per_N_L[:, :, w, d] = (
                                param_spectral_norms
                            )

                    np.save(
                        f"{save_dir}/avg_activity_l1_per_N_L.npy",
                        avg_activity_l1_per_N_L,
                    )
                    np.save(
                        f"{save_dir}/avg_activity_l2_per_N_L.npy",
                        avg_activity_l2_per_N_L,
                    )
                    np.save(
                        f"{save_dir}/param_l2_norms_per_N_L.npy", param_l2_norms_per_N_L
                    )
                    np.save(
                        f"{save_dir}/param_spectral_norms_per_N_L.npy",
                        param_spectral_norms_per_N_L,
                    )
