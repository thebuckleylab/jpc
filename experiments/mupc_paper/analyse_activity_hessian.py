...
import jpc
import numpy as np
from experiments.datasets import get_dataloaders, make_gaussian_dataset
...
def compute_hessian_metrics(
    ...
    if mode == "supervised":
        activities = jpc.init_activities_with_ffwd(
            network, x, skip_model=skip_model, param_type=param_type
        )
    elif mode == "unsupervised":
        activities = jpc.init_activities_from_normal(
            key=key, layer_sizes=layer_sizes, mode=mode, batch_size=1, sigma=1
        )
...
def run_analysis(
    ...
    if weight_init != "standard":
        network = init_weights(key=keys[1], model=network, init_fn_id=weight_init)
    ...
    plot_activity_hessian(metrics["hessian"], f"{save_dir}/hessian_matrix.pdf")
    np.save(f"{save_dir}/num_hessian_eigenvals", metrics["num"])
    np.save(f"{save_dir}/cond_num", metrics["cond_num"])
    if act_fn == "linear":
        np.save(f"{save_dir}/theory_hessian_eigenvals", metrics["theory"])
        np.save(f"{save_dir}/theory_D_eigenvals", metrics["D"])
        np.save(f"{save_dir}/theory_O_eigenvals", metrics["O"])
