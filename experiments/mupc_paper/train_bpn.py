...
from experiments.mupc_paper.utils import init_weights, set_seed, setup_logger
...
def compute_accuracy(model, x, y):
    pred_y = vmap(model)(x)
    return jnp.mean(jnp.argmax(y, axis=1) == jnp.argmax(pred_y, axis=1)) * 100
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for train_iter, (img_batch, label_batch) in enumerate(train_loader):
            ...
            if global_batch_id % test_every == 0:
                ...
                avg_test_loss, avg_test_acc = evaluate(model, test_loader, loss_id)
