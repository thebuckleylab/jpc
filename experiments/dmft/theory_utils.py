...
def solve_kernels(Kx, y, depth, eta, gamma, sigma=1.0, T=100, num_steps=10):
    ...
    for n in range(num_steps):
        ...
        for l in range(depth):
            ...
            H_tril = jnp.einsum(
                "ik,ijkl->ijkl", jnp.tril(jnp.ones((T, T)), k=-1), Hminus
            )
            ...
            Diff_CD_inv = jnp.linalg.inv((I_TP - CD).reshape((T * P, T * P))).reshape(
                (T, P, T, P)
            )

            dH1 = jnp.einsum(
                "ijkl,mnkl->ijmn",
                jnp.einsum("ijkl,klmn->ijmn", Diff_CD_inv, Hminus),
                Diff_CD_inv,
            )
...
def initialize_kernels_sampling(Kx, depth, T, nonlin_fn, dnonlin_fn, samples=1000):
    ...
    for l in range(depth - 2):
        ...
        all_G.insert(
            0,
            all_G[0]
            * draw_nonlin_samples_init(all_Phi[-3 - l], dnonlin_fn, key, samples),
        )
...
def solve_self_consistent_four_vars(
    Phi_minus, G_plus, A, B, Delta, t, r, nonlin_fn, dnonlin_fn, eta_gam, num_step=500
):
    h = t * 1.0
    z = r * 1.0
    ...
    for n in range(num_step):
        ...
        loss = (jnp.sum((h_new - h) ** 2) + jnp.sum((z_new - z) ** 2)) / (
            jnp.sum(h**2) + jnp.sum(z**2)
        )
        h = h_new
        z = z_new
        if loss < 1e-6:
...
def solve_jacobians_batched(
    ...
    for batch_n in range(num_batches):
        ...
        id_batch = jnp.einsum(
            "i,jklm->ijklm",
            jnp.ones(batch_size),
            jnp.einsum("ik,jl->ijkl", jnp.eye(T), jnp.eye(P)),
        )
        ...
        for n in range(num_step):
            ...
            dg_du = jnp.einsum("ijk,ijklm->ijklm", dot_phi_b, dz_du) + jnp.einsum(
                "ijk,ijklm->ijklm", ddot_phi_b * zb, dh_du
            )
            dg_dr = jnp.einsum("ijk,ijklm->ijklm", dot_phi_b, dz_dr) + jnp.einsum(
                "ijk,ijklm->ijklm", ddot_phi_b * zb, dh_dr
            )
...
def solve_kernels_nonlin(
    ...
    if nonlin == "tanh":
        ...
        ddnonlin_fn = lambda h: (
            2 * beta * jnp.tanh(h * beta) * (jnp.tanh(beta * h) ** 2 - 1)
        )

    else:
        nonlin_fn = lambda h: (
            jnp.sqrt(2.0) * 1 / beta * jnp.log(1.0 + jnp.exp(beta * h))
        )
        dnonlin_fn = lambda h: jnp.sqrt(2.0) / (1.0 + jnp.exp(-beta * h))
        ddnonlin_fn = lambda h: (
            jnp.sqrt(2.0) * beta * jnp.exp(-beta * h) / (1.0 + jnp.exp(-beta * h)) ** 2
        )

    # initialize kernels to static init ansatz
    all_Phi, all_G, H0 = initialize_kernels_sampling(
        Kx, depth, T, nonlin_fn, dnonlin_fn
    )
    ...
    for n in range(num_iter):
        ...
        for l in range(depth):
            ...
            h, z, chi, xi = solve_self_consistent_four_vars(
                Phi_minus,
                G_plus,
                A_minus,
                Bl,
            ...
            A_new, B_minus_new = solve_jacobians_batched(
                Phi_minus,
                G_plus,
                A_minus,
                Bl,
        ...
        all_Phi = [
            damping * new_Phi[l] + (1 - damping) * all_Phi[l] for l in range(depth)
        ]
