...
def update_pc_activities(
    ...
    **Returns:**
    ...
    updates, opt_state = optim.update(updates=grads, state=opt_state, params=activities)
...
def update_bpc_params(
    ...
    **Returns:**
    ...
    top_down_updates, top_down_opt_state = top_down_optim.update(
        updates=top_down_grads, state=top_down_opt_state, params=top_down_model
    )
    bottom_up_updates, bottom_up_opt_state = bottom_up_optim.update(
        updates=bottom_up_grads, state=bottom_up_opt_state, params=bottom_up_model
    )
    top_down_model = eqx.apply_updates(model=top_down_model, updates=top_down_updates)
    bottom_up_model = eqx.apply_updates(
        model=bottom_up_model, updates=bottom_up_updates
    )
...
def update_pdm_params(
    ...
    *,
    skip_model: Optional[PyTree[Callable]] = None,
    lateral_model: Optional[PyTree[Callable]] = None,
    lateral_optim: Optional[
        GradientTransformation | GradientTransformationExtraArgs
    ] = None,
    ...
    projection_matrix_prev: Optional[PyTree[Callable]] = None,
    projection_matrix_next: Optional[PyTree[Callable]] = None,
    projection_matrix_prev_optim: Optional[
        GradientTransformation | GradientTransformationExtraArgs
    ] = None,
    projection_matrix_next_optim: Optional[
        GradientTransformation | GradientTransformationExtraArgs
    ] = None,
    ...
    **Returns:**
    ...
    if (
        lateral_model is not None
        and lateral_optim is not None
        and lateral_opt_state is not None
        and lateral_grads is not None
    ):
        lateral_updates, lateral_opt_state = lateral_optim.update(
            updates=lateral_grads, state=lateral_opt_state, params=lateral_model
        )
    ...
    if (
        update_projection_matrix_prev
        and include_previous_backward_error
        and projection_matrix_prev is not None
        and projection_matrix_prev[0] is not None
    ):
        # Use provided optimizer or fall back to top_down_optim
        proj_prev_optim_to_use = (
            projection_matrix_prev_optim
        ...
        projection_matrix_prev[0] = eqx.apply_updates(
            model=projection_matrix_prev[0], updates=proj_prev_updates
        )
    ...
    if (
        update_projection_matrix_next
        and include_next_forward_error
        and projection_matrix_next is not None
    ):
        # Use provided optimizer or fall back to top_down_optim
        proj_next_optim_to_use = (
            projection_matrix_next_optim
        ...
        for i, proj_mat in enumerate(projection_matrix_next):
            if proj_mat is not None:
                ...
                if (
                    projection_matrix_next_opt_states is not None
                    and i < len(projection_matrix_next_opt_states)
                    and projection_matrix_next_opt_states[i] is not None
                ):
                    proj_next_opt_state_to_use = projection_matrix_next_opt_states[i]
                elif (
                    projection_matrix_next_optim is None
                    and i > 0
                    and len(updated_projection_matrix_next_opt_states) > 0
                ):
                    # Use the state from previous projection matrix update when using shared optimizer
                    proj_next_opt_state_to_use = (
                        updated_projection_matrix_next_opt_states[-1]
                        if updated_projection_matrix_next_opt_states[-1] is not None
                        else top_down_opt_state
                    )
                else:
        ...
        if projection_matrix_next_optim is not None:
            result["projection_matrix_next_opt_states"] = (
                updated_projection_matrix_next_opt_states
            )
        else:
            ...
            if (
                updated_projection_matrix_next_opt_states
                and updated_projection_matrix_next_opt_states[-1] is not None
            ):
                result["opt_states"] = (
                    updated_projection_matrix_next_opt_states[-1],
                    bottom_up_opt_state,
...
def update_epc_errors(
    ...
    **Returns:**
    ...
    updates, opt_state = optim.update(updates=grads, state=opt_state, params=errors)
    errors = eqx.apply_updates(model=errors, updates=updates)
    return {"energy": energy, "errors": errors, "grads": grads, "opt_state": opt_state}
