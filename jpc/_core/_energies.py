...
from jax.numpy import array, eye, sqrt, sum
...
def pc_energy_fn(
    ...
    **Returns:**
    ...
    for act_l, net_l in zip(
        range(start_activity_l, n_activity_layers), range(1, n_hidden)
    ):
        err = activities[act_l] - scalings[net_l] * vmap(model[net_l])(
            activities[act_l - 1]
        )
...
def hpc_energy_fn(
    ...
    **Returns:**
    ...
    eL = (
        y - vmap(model[-1])(amort_activities[-2])
        if (y is not None)
    ...
    **Returns:**
    ...
    if hasattr(backward_layer, "layers") and len(backward_layer.layers) == 2:
        ...
        if lateral_model[layer_idx] is not None:
            backward_pred_unactivated += vmap(lateral_model[layer_idx])(
    ...
    if include_previous_backward_error:
        if layer_idx == 0:
            ...
            delta_l_minus_1 = act_prev - vmap(bottom_up_model[layer_idx])(
                activities[layer_idx]
            )
            if skip_model[layer_idx] is not None and layer_idx > 0:
        ...
        if projection_matrix_prev is not None:
            if (
                isinstance(projection_matrix_prev, (list, tuple))
                and len(projection_matrix_prev) > layer_idx
            ):
                delta_l_minus_1_proj = vmap(projection_matrix_prev[layer_idx])(
                    delta_l_minus_1
                )
        ...
        forward_energy = 0.5 * sum(
            (
                forward_energy_weight * e_l
                + backward_energy_weight * delta_l_minus_1_proj
            )
            ** 2
        )
    else:
    ...
    if include_next_forward_error:
        if layer_idx == H - 1:
            ...
            e_l_plus_1 = act_next - vmap(top_down_model[layer_idx + 1])(
                activities[layer_idx]
            )
            if skip_model[layer_idx + 1] is not None and layer_idx < H - 1:
        ...
        if projection_matrix_next is not None:
            if (
                isinstance(projection_matrix_next, (list, tuple))
                and len(projection_matrix_next) > layer_idx
            ):
        ...
        backward_energy = 0.5 * sum(
            (
                backward_energy_weight * delta_l_plus_1
                + forward_energy_weight * e_l_plus_1_proj
            )
            ** 2
        )
    else:
    ...
    **Returns:**
    ...
    if lateral_model is not None:
        ...
        for i in range(H):
            layer = (
                lateral_model[i]
                if isinstance(lateral_model, (list, tuple))
                else lateral_model
            )
            if layer is None:
                continue
            ...
            if (
                ...
                and len(layer.layers) == 2
                and hasattr(layer.layers[1], "weight")
            ):
