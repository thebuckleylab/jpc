...
def test_init_activities_with_ffwd(simple_model, x):
    ...
    activities = init_activities_with_ffwd(model=simple_model, input=x, param_type="sp")
...
def test_init_activities_with_ffwd_mupc(
    key, x, input_dim, hidden_dim, output_dim, depth
):
    ...
    activities = init_activities_with_ffwd(model=model, input=x, param_type="mupc")
...
def test_init_activities_with_ffwd_skip_connections(simple_model, x):
    ...
    activities = init_activities_with_ffwd(
        model=simple_model, input=x, skip_model=skip_model, param_type="sp"
    )
...
def test_init_activities_with_amort(
    key, simple_model, y, input_dim, hidden_dim, output_dim, depth
):
    ...
    activities = init_activities_with_amort(
        amortiser=amortiser, generator=simple_model, input=y
    )
...
def test_init_epc_errors_supervised(layer_sizes, batch_size):
    ...
    errors = init_epc_errors(
        layer_sizes=layer_sizes, batch_size=batch_size, mode="supervised"
    )
