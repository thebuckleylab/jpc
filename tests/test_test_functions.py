...
def test_test_discriminative_pc_cross_entropy(simple_model, x, y_onehot):
    ...
    loss, acc = test_discriminative_pc(
        model=simple_model, output=y_onehot, input=x, loss="ce", param_type="sp"
    )
...
def test_test_generative_pc(
    simple_model, x, y_onehot, key, layer_sizes, batch_size, input_dim
):
    ...
    input_onehot = jax.nn.one_hot(
        jax.random.randint(key1, (batch_size,), 0, input_dim), input_dim
    )
...
def test_test_generative_pc_cross_entropy(
    simple_model, x, y_onehot, key, layer_sizes, batch_size, input_dim
):
